...
public class AdminServiceImpl implements AdminService{
	...
	*/
	...
	/*
	 @Override
	 @Scheduled(cron = "0 28 18 * * ?") // Execute daily at 12 PM
	    public void scheduleTaskReminder() {
	        sendTaskReminders();
	    }

	    private void sendTaskReminders() {
	        List<Task> tasks = taskRepositries.findAll();
	        ...
	        for (Task task : tasks) {
	            ...
	            // Send reminder if the task is due in less than or equal to 1 day
	            if (diffInDays == 1) {
	                User assignee = task.getUser(); // Assuming the task has an assigned user
	                if (assignee != null) {
	                    emailService.sendEmail(assignee.getEmail(), "Task Reminder", "Reminder: Your task \"" + task.getTitle() + "\" is due tomorrow.");
	                }
	            }
	        }
	    }
...
*/
  ...
  /*  
    ...
    public Map<String, Long> getTaskCountByPriorityForAdmin() {
        ...
        Map<String, Long> taskCountByPriority = taskRepositries.countTasksByPriority();
        return taskCountByPriority;
    }

    @Override
    public Map<String, Long> getTaskCountByStatusForAdmin() {
